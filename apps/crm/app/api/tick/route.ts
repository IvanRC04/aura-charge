...
function isAuthorized(req: NextRequest): boolean {
  // Vercel cron jobs send a Bearer token with CRON_SECRET (auto-set by Vercel).
  const auth = req.headers.get("authorization");
  if (process.env.CRON_SECRET && auth === `Bearer ${process.env.CRON_SECRET}`) return true;
  // Allow manual triggers via INTERNAL_SECRET (e.g. running the cron from CLI/CRM page)
  if (process.env.INTERNAL_SECRET && auth === `Bearer ${process.env.INTERNAL_SECRET}`) return true;
  // In local dev, allow without auth
...
export async function GET(req: NextRequest) {
  ...
  for (const s of states) {
    ...
    // Persist a telemetry sample (per tick)
    ...
    // Update cached state with current view (so subsequent reads are warm)
  ...
  // Spawn synthetic activity if too few active sessions (keeps demo lively when nobody is scanning)
  let spawned = 0;
  const targetActive = 2;
  if (states.length - completed < targetActive && Math.random() < 0.55) {
    const spawnRes = await spawnSyntheticSession();
    if (spawnRes) {
      spawned = 1;
    }
  }
...
async function spawnSyntheticSession() {
